...
If you develop with an AI coding agent (Claude Code, Cursor, Copilot, etc.), copy the prompt below and hand it to your agent — it copies the module in and wires it up on its own.

````text
Integrate `android-feature-flag` into THIS project as a local Gradle module (copy the code, no submodule). API reference: https://github.com/ivmakar/android-feature-flag
1. Copy the `feature-flags/` directory from that repo into this project's root.
2. In `settings.gradle.kts` add `include(":feature-flags")`; in `gradle.properties` add `android.experimental.enableTestFixturesKotlinSupport=true`.
3. In the module that owns flags add: `implementation(project(":feature-flags"))` and `testImplementation(testFixtures(project(":feature-flags")))`.
4. Declare a typed flag (`object FavoritesEnabled : BoolFeature("favorites_enabled", default = false)`).
5. Wire `FeatureFlags`: a Hilt module if Hilt is present, else `FeatureFlagsConfigurator(isDebug = BuildConfig.DEBUG, ...)` by hand. Pass `overrideSource` non-null ONLY in debug — `BuildVariantSource`/`PersistentOverrideSource` are debug-only and must never reach a release build.
6. Build the module and run its unit tests; fix the wiring until they pass.
````
