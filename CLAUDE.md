...
**Agent-integration prompt.** Both `README.md` and `README.ru.md` carry a short copy-paste prompt (section "Integrate with a coding agent" / "Подключение через AI-агента") that a consumer hands to their coding agent to wire the library in automatically. It assumes the **copy-the-module** path: copy `feature-flags/` in, `include(":feature-flags")`, depend via `project(":feature-flags")` + `testFixtures(...)`, then wire `FeatureFlags` (Hilt or `FeatureFlagsConfigurator`). **When the module name, the `testFixtures` flag, or the wiring API change**, update the prompt in *both* READMEs to match — a stale prompt sends consuming agents down a broken path. Keep it ≤10 lines and keep the EN and RU prompts semantically identical.
