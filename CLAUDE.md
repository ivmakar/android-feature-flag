...
**Agent-integration prompt.** Both `README.md` and `README.ru.md` carry a copy-paste prompt (section "Integrate with a coding agent" / "Подключение через AI-агента") that a consumer hands to their coding agent to wire the library in automatically. It hardcodes the coordinate, the version, the `includeBuild`/submodule path, the `testFixtures` flag, and the Hilt-vs-manual wiring shape. **When any of those change** (version bump, coordinate, wiring API, testFixtures setup), update the prompt in *both* READMEs to match — a stale prompt sends consuming agents down a broken path. Keep the EN and RU prompts semantically identical.
